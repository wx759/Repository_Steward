      ...
      <div className="mx-auto max-w-4xl">
        <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_10px_30px_rgba(47,54,79,.1)] transition focus-within:border-violet-300 focus-within:ring-2 focus-within:ring-violet-100">
          ...
          <div className="flex items-center justify-between px-1 pb-1">
            ...
            <button className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#8b7cf0] text-white shadow-[0_8px_20px_rgba(104,89,217,.16)] transition hover:bg-[#7a6be3] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none" disabled={disabled || !value.trim()} onClick={submit} aria-label="发送消息" type="button">{disabled ? <LoaderCircle className="animate-spin" size={17} /> : <ArrowUp size={18} />}</button>
          </div>
        </div>
